...
public class Produto {
    ...
    //os valores numéricos são 0 (zero) e os valores de String são "" (nulo)
     ...
     public Produto(int number, String name,int qty,double price)
     {
  ...
  public void addToInventory(int qualidade)
     {
        if(this.active)
           this.qtyInStock += qualidade;
        else
             System.out.println("Não é possivel adicionar estoque a uma linha descontinuada!");
       //endif 
     }//encerrar método addToInventory
     
 //subtrair uma quantidade de qtyInStock ao fazer vendas
 public void deductFromInventory(int qualidade)
     {
         this.qtyInStock-= qualidade;
